...
function useModalDialog(onClose: () => void) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const controls = [...dialog.querySelectorAll<HTMLElement>(focusableSelector)].filter((element) => !element.hidden);
                    ...
                    <div className="divide-y divide-stone-100">
                      {parsed.rows.slice(0, 6).map((row) => (
                        <div key={row.word} className="px-4 py-3">
                          <div className="flex items-center justify-between gap-3">
                            <div><strong className="text-base">{row.word}</strong>{row.phonetic && <span className="ml-2 font-mono text-xs text-blue-700">{row.phonetic}</span>}</div>
    ...
    <>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-card ring-1 ring-stone-100">
        ...
        <div className="mt-7 space-y-3 rounded-2xl bg-blue-50/60 p-4 text-sm leading-6 text-slate-600">
          <p><strong className="text-slate-900">释义：</strong>{current.meaning || "（未填写）"}</p>
          {current.phrase && <p><strong className="text-slate-900">搭配：</strong>{current.phrase}</p>}
          {current.sentence && <p><strong className="text-slate-900">真题句子：</strong>{current.sentence}</p>}
      ...
      <section className="rounded-3xl bg-white p-6 shadow-card ring-1 ring-stone-100">
        ...
        {!spellingChecked ? (
          ...
          <div className="mt-7 rounded-2xl bg-amber-50 p-4 text-amber-950">
            ...
            <button type="button" onClick={retrySpelling} className="button-primary mt-4 w-full">隐藏答案，再拼一次</button>
          </div>
        ) : (
        ...
        <section className="rounded-3xl bg-white p-6 shadow-card ring-1 ring-stone-100">
          ...
          {!spellingChecked ? (
            ...
            <div className="mt-7 rounded-2xl bg-blue-50 p-4 text-blue-900">
              ...
              <button type="button" onClick={continueSpelling} className="button-primary mt-4 w-full">{index === items.length - 1 ? "完成本轮复习" : "下一个词"}</button>
            </div>
          )}
        </section>
      </>
