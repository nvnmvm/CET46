...
function ReviewScheduleChart({ days }: { days: ScheduleDay[] }) {
  ...
  const handleKey = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    setActive(next);
    window.requestAnimationFrame(() => columnRefs.current[next]?.focus());
  };
  return (
    <section className="statistics-card">
      ...
      <div className="schedule-chart" role="group" aria-label="未来十天复习计划">
        {days.map((day, index) => <button ref={(node) => { columnRefs.current[index] = node; }} type="button" key={day.date.toISOString()} className={`schedule-column ${index === 0 ? "is-today" : ""} ${active === index ? "is-active" : ""}`} onClick={() => setActive(index)} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onKeyDown={(event) => handleKey(event, index)} aria-label={`${formatChartDate(day.date)}，需复习 ${day.total} 词`}>
