import { ApiError, apiClient, type ApiReviewDraft, type ApiSettings } from "./apiClient";
import type {
  CountdownSettings,
  LearningEvent,
  LocalStorageStatus,
...
async function listAllWords(userId: string): Promise<WordWithProgress[]> {
  ...
  for (let offset = 0; ; offset += pageSize) {
    const page = await apiClient.listWords(userId, { limit: pageSize, offset });
...
async function hydrate(userId: string): Promise<void> {
  ...
  const [words, events, settings, due, spellingWords, draft] = await Promise.all([
    ...
    apiClient.getDue(userId, 200),
    apiClient.getSpelling(userId, { scope: "daily", limit: 20 }),
    apiClient.getDraft(userId),
  ]);
...
async function loadReviewDraft(userId: string): Promise<ReviewDraft | null> {
  const draft = await apiClient.getDraft(userId);
...
export const cloudRepository = {
  ...
  async killWord(userId: string, wordId: string): Promise<boolean> {
    invalidateHydration();
    const progress = await apiClient.progressAction(userId, wordId, "kill");
    replaceProgress(userId, progress);
    return true;
  },
  async restoreWord(userId: string, wordId: string): Promise<boolean> {
    invalidateHydration();
    const progress = await apiClient.progressAction(userId, wordId, "restore");
    replaceProgress(userId, progress);
    return true;
  },
  async undoKillWord(userId: string, wordId: string): Promise<boolean> {
    invalidateHydration();
    const progress = await apiClient.progressAction(userId, wordId, "undo-kill");
    replaceProgress(userId, progress);
    return true;
  },
  ...
  async completeNewStudy(userId: string, entries: WordWithProgress[], options?: { submissionKey?: string }) {
    ...
    const result = await apiClient.completeNewStudy(userId, entries, {
  ...
  async finishReview(
    ...
    const result = await apiClient.finishReview(userId, entries, recognition, spelling, {
      submissionKey: options?.submissionKey ?? submissionKeyFor(signature),
    });
    await hydrate(userId);
    completionKeys.delete(signature);
  ...
  async finishSpellingPractice(
    ...
    const result = await apiClient.finishSpellingPractice(userId, entries, spelling, {
      submissionKey: options?.submissionKey ?? submissionKeyFor(signature),
    });
    await hydrate(userId);
    completionKeys.delete(signature);
