...
`003_admin_accounts_mysql.sql` 新增：

- `users.role`（默认 `learner`）与 `users.disabled_at`；禁用账号不能解析已有会话。
- `admin_audit_logs`：只记录管理员、目标账号、动作和时间，不记录密码、摘要或学习内容。

管理员 API 只允许服务端会话角色为 `admin` 的账号调用；本轮不开放角色编辑、删除或公开注册。
首个管理员必须由受控的 `server/scripts/promote-user.ts --email someone@example.com` 人工提升，
不会自动操作线上账号。运行期 MySQL 账号应只拥有业务表 CRUD 权限，迁移账号单独授予 DDL 权限。
