  ...
  return {
    ...
    async findUserByEmail(email) {
      const [rows] = await pool.execute<UserRow[]>(
        "SELECT id, email, password_hash, username, avatar, timezone, created_at, updated_at FROM users WHERE email = ? LIMIT 1",
        [email],
      );
      const row = rows[0];
    ...
    async findUserById(id) {
      const [rows] = await pool.execute<UserRow[]>(
        "SELECT id, email, password_hash, username, avatar, timezone, created_at, updated_at FROM users WHERE id = ? LIMIT 1",
        [id],
      );
      const row = rows[0];
      return row ? mapUser(row) : null;
    ...
    async findActiveSessionByTokenHash(tokenHash, now) {
      const [rows] = await pool.execute<(SessionRow & UserRow)[]>(
        `SELECT s.id, s.user_id, s.token_hash, s.expires_at, s.revoked_at, s.created_at,
                u.email, u.password_hash, u.username, u.avatar, u.timezone,
