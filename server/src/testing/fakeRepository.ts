...
export type FakeRepository = WordRepository & {
  ...
  snapshotAdminAudit(): { actorUserId: string; targetUserId: string; action: string }[];
  /**
...
export function createFakeRepository(options: FakeRepositoryOptions = {}): FakeRepository {
  ...
  const adminAudit: { id: string; actorUserId: string; targetUserId: string; action: string; createdAt: string }[] = [];
  ...
  const repository: FakeRepository = {
    ...
    async adminCreateUser(actorUserId, input) {
      ...
      const email = input.email.trim().toLowerCase();
      if (userIdByEmail.has(email)) throw new RepositoryConflictError("email_exists");
      const created = this.seedUser({ ...input, email, role: "learner" });
      adminAudit.push({ id: newId("audit"), actorUserId, targetUserId: created.id, action: "create_user", createdAt: nowIso() });
      return created;
    },
    ...
    async adminSetUserDisabled(actorUserId, targetUserId, disabled) {
      ...
      const target = users.get(targetUserId);
      if (!target) return null;
      if (disabled && targetUserId === actorUserId) throw new RepositoryForbiddenError("cannot_disable_self");
      if (disabled && target.role === "admin") throw new RepositoryForbiddenError("cannot_disable_admin");
      const timestamp = nowIso();
      target.disabledAt = disabled ? timestamp : null;
      target.updatedAt = timestamp;
      if (disabled) {
        for (const session of sessions.values()) if (session.userId === targetUserId && session.revokedAt === null) session.revokedAt = timestamp;
      }
      adminAudit.push({ id: newId("audit"), actorUserId, targetUserId, action: disabled ? "disable_user" : "enable_user", createdAt: timestamp });
      return { ...target };
    },
    ...
    async adminResetUserPassword(actorUserId, targetUserId, passwordHash) {
      ...
      target.updatedAt = timestamp;
      for (const session of sessions.values()) if (session.userId === targetUserId && session.revokedAt === null) session.revokedAt = timestamp;
      adminAudit.push({ id: newId("audit"), actorUserId, targetUserId, action: "reset_password", createdAt: timestamp });
      return { ...target };
    },
    ...
    async createSession(input: NewSessionInput): Promise<SessionRecord> {
      if (input.expectedPasswordHash !== undefined) {
        ...
        if (!user || user.disabledAt !== null || user.passwordHash !== input.expectedPasswordHash) {
    ...
    async findActiveSessionByTokenHash(
      ...
      return user && user.disabledAt === null ? { session, user } : null;
    },
    ...
    snapshotAdminAudit() {
      return adminAudit.map(({ actorUserId, targetUserId, action }) => ({ actorUserId, targetUserId, action }));
    },
