...
### 8.3 本次 1Panel 实测补记（2026-09-20）

1Panel 2.3.1 的 Node.js 创建页已实测支持 Node 24.15.0、pnpm、项目目录和自定义启动命令。当前页面的端口配置只有“外部访问”开关，没有把宿主机绑定地址设为 `127.0.0.1` 的字段；而 Alice 现有 OpenResty 使用宿主机网络。因此备用 Node 实例若打开 3001 会直接产生公网监听，关闭则宿主机 OpenResty 无法访问。

本次没有提交该不安全配置，旧 API 保持运行；临时复制的服务器环境文件已删除。下一次迁移必须先采用已验证的 host 网络/本机回环绑定，或让 1Panel 反代进入同一私有网络，再创建运行实例。不能仅凭“Node 页面创建成功”宣称 API 已迁移。
